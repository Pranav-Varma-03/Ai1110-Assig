...
\textbf{Median $(M)$} : If $n$ be the number of entries in given data then median of the data is given by ,\\

	\begin{enumerate}[label=(\roman*)]
		\item if $n$ = $odd$ 
		\begin{center}
		    $ M= \left(\dfrac{n}{2}\right)^{th}$ element
		\end{center}
		
		\item if $n$ = $even$
		\begin{center}
		    $ M= \dfrac{{\left(\dfrac{n}{2}\right)}^{th}+\left(\dfrac{n+1}{2}\right)^{th}}{2}$ \\[10pt]
		\end{center} 
	\end{enumerate}
As here the value of n is \textbf{odd}\\
	From (i) ,
	\begin{align*}
		 M &= \dfrac{(x)+(x+4)}{2}\\
	 	 M &= \dfrac{(2x+4)}{2}\\
...
Converting given set of sorted numbers into Class Intervals ,\\[6pt]
DATA : $13, 35, 43, 46, 46, 50, 55, 61, 71, 80$\\[6pt]
%table
TABLE :
	\begin{table}[H]
...
given problem,\\
	\begin{align}
		...
		\vec{S} = \myvec{50 \\ 2}
 	\end{align}
and from $(3),(4)$ the desired mode is
	\begin{align*}
